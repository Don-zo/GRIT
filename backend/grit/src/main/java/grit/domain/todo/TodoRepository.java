...
public interface TodoRepository extends JpaRepository<Todo, Long> {
    ...
    @Query("SELECT t FROM Todo t JOIN FETCH t.group g JOIN FETCH t.owner WHERE g.id = :groupId")
    List<Todo> findByGroupIdWithRelations(@Param("groupId") Long groupId);

    @Query("SELECT t FROM Todo t JOIN FETCH t.group g JOIN FETCH t.owner WHERE g.id = :groupId AND t.owner.id = :ownerId")
    List<Todo> findByGroupIdAndOwnerIdWithRelations(@Param("groupId") Long groupId, @Param("ownerId") Long ownerId);

    @Query("SELECT t FROM Todo t LEFT JOIN FETCH t.group g JOIN FETCH t.owner WHERE t.owner.id = :ownerId")
