...
class StudyTimeServiceTest {
    ...
    void duplicateResumeAndPauseDoNotDoubleCountElapsedSeconds() {
        studyTimeService.resume(member);
        clock.setInstant(Instant.parse("2026-07-06T00:10:00Z"));
        studyTimeService.resume(member);

        clock.setInstant(Instant.parse("2026-07-06T00:20:00Z"));
        studyTimeService.pause(member);
        studyTimeService.pause(member);

        assertThat(studyTimeService.getStudyTime(member).currentWeekStudyTimeSeconds()).isEqualTo(20 * 60);
    }

    @Test
    void roomLeavePausesPersonalTimer() {
        studyTimeService.resume(member);

        studyTimeService.pauseForRoomLeave(member, Instant.parse("2026-07-06T00:10:00Z"));

        assertThat(studyTimeService.getStudyTime(member).currentWeekStudyTimeSeconds()).isEqualTo(10 * 60);
    ...
    @Test
    void pauseWithoutRunningTimerDoesNotCreateIdleTimerState() {
        studyTimeService.pause(member);
    ...
    @Test
    void resumeKeepsRunningAfterStudyTimeLookup() {
        studyTimeService.resume(member);

        assertThat(studyTimeService.getStudyTime(member).running()).isTrue();
    }

    @Test
    void getStudyTimeReturnsGoalAndCurrentWeekAccumulatedSeconds() {
        studyTimeService.resume(member);
        clock.setInstant(Instant.parse("2026-07-06T00:30:00Z"));
        studyTimeService.pause(member);

        MemberStudyTimeResponseDto response = studyTimeService.getStudyTime(member);

        assertThat(response.weeklyStudyTimeGoalSeconds()).isEqualTo(12 * 3600 + 30 * 60L);
        assertThat(response.currentWeekStudyTimeSeconds()).isEqualTo(30 * 60);
        assertThat(response.weekStartDate()).isEqualTo(LocalDate.parse("2026-07-06"));
